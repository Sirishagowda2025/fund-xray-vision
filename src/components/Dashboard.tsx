...
const StatusBadge=({status}:{status:Fund["status"]})=>{
  const s={Outperforming:"bg-green-50 text-green-700 border border-green-200",Underperforming:"bg-red-50 text-red-700 border border-red-200",Watch:"bg-amber-50 text-amber-700 border border-amber-200"};
  return <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${s[status]}`}>{status}</span>;
};
...
export default function Dashboard({data,userName,userProfile,uploadWarning,onAnalyseAnother,onExit,onLogout}:Props) {
  ...
  const handleDownload=()=>{
    ...
    if(userProfile?.healthDimensions){
      ...
      L.push("MONEY HEALTH SCORE"); L.push("-".repeat(30));
      L.push(`Overall: ${overall}/100`);
      DIMS.forEach(({key,label})=>L.push(`  ${label}: ${d[key as keyof typeof d]}/100`));
      L.push("");
    }
    L.push("PORTFOLIO SUMMARY"); L.push("-".repeat(30));
    L.push(`Health Score:  ${metrics.healthScore}/100`);
    L.push(`XIRR:          ${metrics.xirr}%`);
    L.push(`Total Value:   ${fmt(metrics.totalValue)}`);
    L.push(`Overlap:       ${metrics.portfolioOverlap}%`);
    L.push(`Expense Drag:  ${fmt(metrics.annualExpenseDrag)}/yr`);
    L.push(""); L.push("YOUR FUNDS"); L.push("-".repeat(30));
    ...
    a.href=url; a.download=`portfolio-xray-${(userName||"report").toLowerCase().replace(/\s+/g,"-")}-${new Date().toISOString().split("T")[0]}.txt`;
  ...
  return(
    <div className="min-h-screen bg-gray-50 pb-28">
      {/* Sticky navbar */}
      <nav className="sticky top-0 z-40 bg-white/90 backdrop-blur-md border-b border-gray-200 px-6 py-3">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {userName&&<span className="text-xs text-gray-500 hidden sm:block font-medium">{userName}</span>}
            {onExit&&(
              <button onClick={onExit} className="flex items-center gap-1.5 text-xs text-gray-600 hover:text-gray-900 bg-white border border-gray-200 px-3 py-1.5 rounded-lg hover:bg-gray-50 transition-all">
                <X className="h-3.5 w-3.5"/>Exit
              </button>
            )}
            {onLogout&&(
              <button onClick={onLogout} className="flex items-center gap-1.5 text-xs text-gray-600 hover:text-gray-900 bg-white border border-gray-200 px-3 py-1.5 rounded-lg hover:bg-gray-50 transition-all">
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <motion.div initial={{opacity:0,y:-10}} animate={{opacity:1,y:0}} className="text-center mb-10">
          <h1 className="text-3xl font-bold text-gray-900 mb-1" style={{fontFamily:"'Space Grotesk',sans-serif"}}>
            {userName?`${userName}'s Portfolio X-Ray`:"Your Portfolio X-Ray"}
          </h1>
          <p className="text-gray-500 text-sm">AI-powered analysis · Powered by Claude · ET Gen AI Hackathon 2026</p>
        </motion.div>
        ...
        {uploadWarning&&(
          <div className="flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 mb-6 text-sm text-amber-700">
            ...
            <span>Couldn't fully read your PDF — showing demo data. Try a CAMS consolidated statement.</span>
          </div>
        )}
        ...
        {userProfile?.healthDimensions&&(
          <section className="mb-8">
            <h2 className="text-xl font-bold text-gray-900 mb-1" style={{fontFamily:"'Space Grotesk',sans-serif"}}>Your Money Health Score</h2>
            <p className="text-sm text-gray-500 mb-4">6-dimension financial wellness · personalised to {userName||"your"} profile</p>
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
              <div className="flex items-center gap-4 mb-5">
                <div className="text-5xl font-bold text-blue-600" style={{fontFamily:"'Space Grotesk',sans-serif"}}>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {DIMS.map(({key,label})=>{
                  ...
                  const bar=v>=75?"bg-green-500":v>=50?"bg-amber-500":"bg-red-500";
                  const bg=v>=75?"bg-green-50 border-green-100":v>=50?"bg-amber-50 border-amber-100":"bg-red-50 border-red-100";
                  return(
                    <div key={key} className={`p-3 rounded-xl border ${bg}`}>
                      <div className="text-xs text-gray-500 mb-1">{label}</div>
                      <div className="text-xl font-bold text-gray-900 mb-1.5" style={{fontFamily:"'Space Grotesk',sans-serif"}}>{v}<span className="text-xs font-normal text-gray-400">/100</span></div>
        ...
        {/* Funds table */}
        <section className="mb-8">
          <h2 className="text-xl font-bold text-gray-900 mb-4" style={{fontFamily:"'Space Grotesk',sans-serif"}}>Your Funds</h2>
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50/80">
                    <th className="text-left px-4 py-3 font-semibold text-gray-500 text-xs uppercase tracking-wide">Fund Name</th>
                    <th className="text-left px-4 py-3 font-semibold text-gray-500 text-xs uppercase tracking-wide hidden md:table-cell">Category</th>
                    <th className="text-right px-4 py-3 font-semibold text-gray-500 text-xs uppercase tracking-wide">Invested</th>
                    <th className="text-right px-4 py-3 font-semibold text-gray-500 text-xs uppercase tracking-wide">Current</th>
                    <th className="text-right px-4 py-3 font-semibold text-gray-500 text-xs uppercase tracking-wide hidden sm:table-cell">XIRR</th>
                ...
                <tbody>
                  {funds.map((fund,i)=>(
                    <motion.tr key={fund.name} initial={{opacity:0}} animate={{opacity:1}} transition={{delay:i*0.05}}
                      ...
                      <td className="px-4 py-3.5 font-medium text-gray-900">{fund.name}</td>
                      <td className="px-4 py-3.5 text-gray-500 hidden md:table-cell">{fund.category}</td>
                      <td className="px-4 py-3.5 text-right text-gray-700">{fmt(fund.amountInvested)}</td>
                      <td className="px-4 py-3.5 text-right font-semibold text-gray-900">{fmt(fund.currentValue)}</td>
                      <td className="px-4 py-3.5 text-right font-bold text-gray-900 hidden sm:table-cell">{fund.xirr}%</td>
        ...
        {/* AI Insights */}
        <section className="mb-8">
          <h2 className="text-xl font-bold text-gray-900 mb-4 flex items-center gap-2" style={{fontFamily:"'Space Grotesk',sans-serif"}}>
            <Lightbulb className="h-5 w-5 text-blue-600"/>AI Insights
          </h2>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-gray-200 px-4 py-4 z-40">
        <div className="max-w-5xl mx-auto flex flex-col sm:flex-row justify-center gap-3">
          <button onClick={handleDownload}
            className="flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 active:scale-[0.98] text-white font-semibold text-sm px-8 py-3 rounded-xl transition-all shadow-md shadow-blue-100">
            <Download className="h-4 w-4"/>Download Full Report
          </button>
          {onAnalyseAnother&&(
            <button onClick={onAnalyseAnother}
              className="flex items-center justify-center gap-2 text-gray-700 hover:text-gray-900 border border-gray-200 hover:border-gray-300 bg-white font-semibold text-sm px-8 py-3 rounded-xl transition-all">
      ...
      {/* AI Chat — bottom-20 so it clears the sticky bar */}
